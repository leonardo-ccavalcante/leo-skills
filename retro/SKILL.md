...
description: |
  ...
  metrics with persistent history and trend tracking. Team-aware: breaks down per-person
  contributions with praise and growth areas. Use this skill whenever the user says "weekly retro",
  "what did we ship", "engineering retrospective", "let's retro", "weekly review", or invokes
  /retro. Proactively suggest at the end of a work week or sprint.
---
...
First, detect the default branch (use `git symbolic-ref refs/remotes/origin/HEAD | sed 's|refs/remotes/origin/||'` or fall back to `main`/`master`), fetch origin, and identify the current user:
...
```
| Backlog Health | N open (X P0/X1, Y P2) · Z completed this period |
```
...
**If there are Co-Authored-By trailers:** Parse `Co-Authored-By:` lines in commit messages. Credit those authors for the commit alongside the primary author. Note AI co-authors but do not include them as team members — instead, track "AI-assisted commits" as a separate metric.
...
**Note:** Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found. If any has no data, omit the field entirely.
...
### Test Health
- Total test files: N
- Tests added this period: M
- Regression test commits: list `test(qa):` and `test(design):` and `test: coverage` commits
- If prior retro exists and has `test_health`: show delta "Test count: {last} → {now} (+{delta})"
- If test ratio < 20%: flag as growth area
...
### Global Step 2: Discover repos with recent activity

Find all git repos on the machine with commits in the time window. (Use your standard workflow tools — `find ~ -name .git -type d` filtered to repos with recent activity, or a project-tracking tool you have available.)

If no AI coding sessions or recent repo activity is found, say: "No coding activity found in the last <window>. Try a longer window: `/retro global 30d`" and stop.

### Global Step 3: Run git log on each discovered repo

For each repo, find the path (directory exists with `.git/`). If no valid path exists, skip the repo and note it.
...
Detect the default branch for each repo: first try `git symbolic-ref refs/remotes/origin/HEAD`, then check common branch names (`main`, `master`), then fall back to `git rev-parse --abbrev-ref HEAD`.
...
# PR/MR numbers from commit messages
...
### Global Step 6: Aggregate and generate narrative
...
```
Week of Mar 14: 5 projects, 138 commits, 250k LOC across 5 repos | Streak: 52d
```
...
Render as a single visually clean block. Left border only — no right border. Pad repo names to the longest name so columns
...
- **Never truncate repo names.** Pad the name column to the longest repo name so all columns align.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from commit messages.
...
For each project, add a "Your contributions" block showing the current user's personal stats within that repo. Include:
...
If the user has 0 commits in a repo, say "No commits this period." and skip the breakdown.
...
### Global Step 7: Load history & compare

```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.retros/global-*.json 2>/dev/null | head -5
```
...
If a matching prior retro exists, load it and show a **Trends vs Last Global Retro** table with deltas for key metrics: total commits, LOC, streak, context switches/day.

If no prior global retros exist, append: "First global retro recorded — run again next week to see trends."

### Global Step 8: Save snapshot

```bash
mkdir -p ~/.retros
today=$(date +%Y-%m-%d)
existing=$(ls ~/.retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Save JSON to `~/.retros/global-${today}-${next}.json` with this schema:
...
```json
{
  ...
  "projects": [
    {
      "name": "myrepo",
      "remote": "<detected from git remote get-url origin>",
      "commits": 47,
      "insertions": 3200,
      "deletions": 800
    }
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | Focus: myrepo (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.retros/`. If streak hits 365d cap, display as "365+ days".
