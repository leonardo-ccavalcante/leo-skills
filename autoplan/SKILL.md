...
name: autoplan
description: |
  Auto-review pipeline — runs the full CEO, design, eng, and DX plan reviews sequentially
  with auto-decisions using 6 decision principles. Surfaces taste decisions (close
  approaches, borderline scope, model disagreements) at a final approval gate. One
  command, fully reviewed plan out. Use this skill whenever the user asks to "auto
  review", "autoplan", "auto plan", "automatic review", "run all reviews", "review this
  plan automatically", or "make the decisions for me". Also triggers on /autoplan.
  Proactively invoke when the user has a plan file and wants to run the full review
...
manually. The only difference: intermediate questions are auto-decided using
...
3. **Model disagreements** — outside-voice model recommends differently and has a valid point.

**User Challenge** — both models agree the user's stated direction should change.
This is qualitatively different from taste decisions. When the primary review and the outside voice both
...
**Exception:** If both models flag the change as a security vulnerability or feasibility blocker (not a preference), the question framing explicitly warns: "Both models believe this is a security/feasibility risk, not just a preference." The user still decides, but the framing is appropriately urgent.
...
2. User Challenges — when both models agree the user's stated direction should change (merge, split, add, remove features/workflows). The user always has context models lack.
...
All prompts sent to outside-voice models (e.g., Codex via `codex exec`) MUST be prefixed with this boundary instruction:

> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories. These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents the outside model from discovering skill files on disk and following their instructions instead of reviewing the plan.
...
Before doing anything, save the plan file's current state to an external file so it can be restored if the review goes wrong. Write the plan's contents with this header:
...
- Read CLAUDE.md, TODOS.md, `git log -30`, `git diff` against the base branch with `--stat`
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST, GraphQL, gRPC, webhook, CLI, command, flag, argument, terminal, shell, SDK, library, package, npm, pip, import, require, SKILL.md, skill template, Claude Code, MCP, agent, developer docs, getting started, onboarding, integration, debug, implement, error message). Require 2+ matches. Also trigger DX scope if the product IS a developer tool or if an AI agent is the primary user.

### Step 3: Load skill files from disk

Read each file using the Read tool:
- plan-ceo-review skill
- plan-design-review skill (only if UI scope detected)
- plan-eng-review skill
- plan-devex-review skill (only if DX scope detected)
...
Before invoking any outside voice (e.g., Codex), verify the CLI is available and authenticated.

```bash
if ! command -v codex >/dev/null 2>&1; then
  echo "[codex-unavailable: binary not found] — proceeding with subagent only"
  _CODEX_AVAILABLE=false
elif ! codex auth status >/dev/null 2>&1; then
  echo "[codex-unavailable: auth missing] — proceeding with subagent only. Run \`codex login\` or set \$CODEX_API_KEY to enable dual-voice review."
  _CODEX_AVAILABLE=false
else
...
If `_CODEX_AVAILABLE=false`, all phase Codex voices below degrade to `[codex-unavailable]` in the degradation matrix. /autoplan completes with the Claude subagent only.

---

## Phase 1: CEO Review (Strategy & Scope)

Follow plan-ceo-review at full depth.
...
- Dual voices: always run BOTH a Claude subagent AND Codex (or other outside model) if available (P6). Run them sequentially in foreground. First the Claude subagent (Agent tool), then Codex. Both must complete before building the consensus table.

  **Codex CEO voice** (via Bash):
  ```bash
  codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories. Stay focused on repository code only.
  ...
  File: <plan_path>" -s read-only --enable web_search_cached < /dev/null
  ```
  Timeout: 10 minutes. On hang, auto-degrades this phase's Codex voice.
...
- Strategy choices: if codex disagrees with a premise or scope decision with valid strategic reason → TASTE DECISION. If both models agree the user's stated structure should change → USER CHALLENGE (never auto-decided).
...
Step 0.5 (Dual Voices): Run Claude subagent first, then Codex. Produce CEO consensus table:
...
- Design section: run only if UI scope was detected in Phase 0
...
Follow plan-design-review at full depth across all 7 dimensions.
Override: every question → auto-decide using the 6 principles.
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex design voice** prompt template:
  "IMPORTANT: filesystem boundary as above.
  Read the plan file at <plan_path>. Evaluate this plan's UI/UX design decisions.

  Also consider these findings from the CEO review phase:
  <insert CEO dual voice findings summary>

  Does the information hierarchy serve the user or the developer? Are interaction states (loading, empty, error, partial) specified or left to the implementer's imagination? Is the responsive strategy intentional or afterthought? Are accessibility requirements specified or aspirational? Does the plan describe specific UI decisions or generic patterns? What design decisions will haunt the implementer if left ambiguous? Be opinionated. No hedging."
...
2. Step 0.5 (Dual Voices): Run Claude subagent first, then Codex. Produce design litmus scorecard (consensus table). Include CEO phase findings in Codex prompt ONLY (not Claude subagent — stays independent).
...
Follow plan-eng-review at full depth.
Override: every question → auto-decide using the 6 principles.
...
2. Step 0.5 (Dual Voices): Run Claude subagent first, then Codex. Produce eng consensus table:
...
Follow plan-devex-review at full depth across all 8 DX dimensions.
Override: every question → auto-decide using the 6 principles.
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex DX voice** prompt template:
  "IMPORTANT: filesystem boundary as above.
...
2. Step 0.5 (Dual Voices): Run Claude subagent first, then Codex. Produce DX consensus table:
...
3. Passes 1-8: Run each from loaded skill. Rate 0-10. Auto-decide each issue. DISAGREE items from consensus table → raised in the relevant pass with both perspectives.
...
[If security/feasibility: "WARNING: Both models flag this as a security/feasibility risk,
...
[Render the aggregated task list across all phases that ran. Deduplicate by (component,
sorted files, title). Sort by priority (P1 > P2 > P3) then phase order (CEO, design,
eng, DX). If no tasks were emitted, render: "_No actionable tasks emitted from any phase._"]
```
...
On approval, suggest the next step: `/ship` when ready to create the PR.
...
- **Two gates.** The non-auto-decided questions are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
