...
name: canary
description: |
  Post-deploy canary monitoring. Watches the live app for console errors, performance regressions,
  and page failures using a headless browser. Takes periodic screenshots, compares against
  pre-deploy baselines, and alerts on anomalies. Use this skill whenever the user says "monitor
  deploy", "canary", "post-deploy check", "watch production", "verify deploy", "monitor after
  deploy", "watch for errors post-deploy", or invokes /canary. Proactively suggest immediately
  after a production deploy completes.
---
...
You use a headless browser tool (e.g., Playwright, Puppeteer, or any browser automation CLI you have available — referenced below as `browse`) to watch the live app, take screenshots, check console errors, and compare against baselines. You are the safety net between "shipped" and "verified."
...
This skill requires a headless browser tool capable of:
- Navigating to URLs
- Taking screenshots
- Reading console errors
- Reporting page load timing (e.g., via the `performance` API)
- Extracting page links

This could be Playwright, Puppeteer, the gstack `browse` CLI, or any equivalent. Commands below reference `$B` as the path to whatever browser tool is available. Substitute with the actual command (e.g., `npx playwright`, `puppeteer-cli`, etc.).
...
Extract the top 5 internal navigation links from the `links` output. Always include the homepage. Present the page list to the user:

- **Context:** Monitoring the production site at the given URL after a deploy.
- **Question:** Which pages should the canary monitor?
- **Recommendation:** Monitor the discovered navigation targets.
...
- **Recommendation:** Choose based on severity — investigate for critical, wait for transient.
...
- **Recommendation:** Update baseline — deploy is healthy, new baseline reflects current production.
