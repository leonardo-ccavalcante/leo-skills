...
description: |
  ...
  builds a Diataxis coverage map (reference/how-to/tutorial/explanation), updates
  ...
  optionally bumps VERSION. Surfaces documentation debt in the PR body. Use this skill
  whenever the user says "update the docs", "sync documentation", "post-ship docs",
  "document what changed", "docs after ship", or invokes /document-release. Proactively
  suggest after a PR is merged or code is shipped.
---

# Document Release: Post-Ship Documentation Update

You are running the `/document-release` workflow. This runs **after code is committed and a PR
exists or is about to exist** but **before the PR merges**. Your job: ensure every documentation file
...
- Bump VERSION without asking — always confirm before version changes
...
applied as an audit lens, not a generation tool.
...
1. **Extract public surface changes from the diff.** Scan `git diff <base>...HEAD` for:
   ...
   - New workflows or user-facing capabilities
...
Coverage map:
  ...
  /new-thing       Yes README   No        No          No
  --new-flag       Yes README   Yes README No          No
  FooProcessor     No           No        No          No
```

Use these definitions:
- **Reference** — factual description of what it is, its API, its options
...
- A clear recommendation with one-line reason
...
3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), ask the user
   whether it should be captured in TODOS.md.
...
3. **If VERSION was NOT bumped:** Ask the user:
   - Recommend: Skip — docs-only changes rarely warrant a version bump
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   b. Read the full diff. Are there significant changes (new features, new commands, major refactors)
      that are NOT mentioned in the CHANGELOG entry for the current version?
   c. **If the CHANGELOG entry covers everything:** Skip — output "VERSION: Already bumped to
      vX.Y.Z, covers all changes."
   d. **If there are significant uncovered changes:** Ask the user, explaining what the
      current version covers vs what's new.
...
3. The Documentation section should include:
   ...
   a. **Doc diff preview** — for each file modified, describe what specifically changed.
...
**If GitLab:** Use `glab mr update -d "$(cat /tmp/pr-body-$$.md)"` with a heredoc to avoid shell metacharacter issues.
...
PR titles must always start with `v<VERSION>`. If Step 8 bumped VERSION after the PR was already created, the title is now stale. This sub-step fixes it.
...
3. Compute the corrected title. Three cases: title already correct (no-op), title has a different `v<X.Y.Z>` prefix (replace it), or title has no version prefix (prepend one).

4. If the new title differs from the current title, update it:
...
5. If the edit command fails: warn and continue. Do not block on title sync failure.
...
If the coverage map from Step 1.5 identified any gaps, append a coverage table and any
diagram drift findings. If all coverage is complete and no diagrams drifted, output:
"Coverage: all shipped features have adequate documentation."
...
- **Coverage map informs, never generates.** The Diataxis coverage map flags gaps for the PR body
  and future work. It does NOT auto-generate missing documentation pages or sections.
