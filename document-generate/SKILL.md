...
description: |
  Generate missing documentation from scratch for a feature, module, or entire project. Uses the
  Diataxis framework (tutorial / how-to / reference / explanation) to produce complete, structured
  documentation. Use this skill whenever the user says "write docs", "generate documentation",
  "document this feature", "create a tutorial", "write a how-to", "explain this module", "docs for
  this project", or invokes /document-generate. Proactively suggest when a coverage gap is detected
...
2. **From a doc audit** — a coverage map identified gaps; you fill them
...
1. Determine what to document:
   - **If invoked with a specific target** (feature, module, file): scope is that target
   - **If invoked for an entire project**: scope is the full project
   - **If called for specific gaps**: scope is those specific entities

2. Confirm scope and ask about documentation target:
   ...
   Recommend C because it maximizes both discoverability and depth.
...
If the plan has more than 5 documents to create, confirm with the user before proceeding.
...
- **Completeness over minimalism.** Don't write "minimal viable docs" — write complete docs.
